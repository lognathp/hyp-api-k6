...
show_help() {
    ...
    echo "    --prometheus URL   Publish results to Prometheus remote write endpoint"
    echo "    --prometheus-auto  Use default Prometheus (${PROMETHEUS_HOST}:${PROMETHEUS_PORT})"
    echo ""
    echo -e "${CYAN}Examples:${NC}"
    echo "    $0 smoke --restaurant 324672"
    echo "    $0 smoke --restaurant 324672 --prometheus-auto"
    echo "    $0 load --restaurant 324672 --cloud YOUR_TOKEN"
    echo "    $0 load --restaurant 324672 --prometheus http://VPS_IP:9090/api/v1/write"
